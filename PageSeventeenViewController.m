...
@interface PageSeventeenViewController (ViewHandlingMethods)
- (void)prevController;
- (void)nextController;
...
@implementation PageSeventeenViewController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    self.title = NSLocalizedString(@"Site 17", @"");
...
- (void) nextController {
    ...
    PageEighteenViewController *pageeighteenVC = [[PageEighteenViewController alloc] initWithNibName: @"PageEighteenViewController" bundle:nil];
    // Copy the view controllers stack from the navigation controller
    NSMutableArray *navigationStack = [[NSMutableArray alloc] initWithArray:self.navigationController.viewControllers];
    ...
    // Add the next view controller
    pageeighteenVC.hidesBottomBarWhenPushed = YES;
    [navigationStack addObject:pageeighteenVC];
    
    [pageeighteenVC release];
...
-(void)loadAudioFile {
    ...
    // Load the audio file
    if(player == nil){
        NSString *audioFile = [NSString stringWithFormat:@"nolac17"];
